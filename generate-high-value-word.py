...
while True:
    ...
    regex_pattern = re.compile('^' + ''.join([bad_letters if RE_NOT_ALPHA.match(letter) else letter for letter in word_pattern]) + '$')

    # Filter the word list based on the regex pattern and good letters
    filtered_words = (word for word in word_list[word_length] if regex_pattern.match(word) and all(letter in word for letter in required_letters))
